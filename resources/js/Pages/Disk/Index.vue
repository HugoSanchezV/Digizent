...
const filteredDisks = computed(() => {
  ...
  return props.disks.filter(
    (disk) =>
      disk.name.toLowerCase().includes(query) ||
      disk.artist.name.toLowerCase().includes(query)
  );
});
</script>

<template>
  <AppLayout title="Diks">
    <template #header>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  >
                    ...
                    <td
                      class="px-4 py-3 whitespace-nowrap text-sm text-gray-800"
                    >
                      <Link :href="route('disk.show', disk.id)">
                        <div
                          class="py-1 px-2 bg-indigo-400 text-white rounded-md cursor-pointer"
                        >
                          stores
                        </div>
                      </Link>
